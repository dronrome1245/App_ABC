...
## 16. ОБЯЗАТЕЛЬНЫЙ DOD EVIDENCE MATRIX ПЕРЕД ЗАКРЫТИЕМ MILESTONE

Это обязательный anti-regression guardrail. Его нельзя заменять общей фразой «всё протестировано» или `OWNER_ACCEPTANCE_STATUS: ACCEPTED`.

Перед тем как:

- написать `M1/M2/... DONE`, `100%`, `COMPLETED`;
- сформировать task packet на merge closure/release PR;
- разрешить переход к следующему milestone;
- предложить обновить `PROJECT_STATUS.md` как завершённый этап,

ты обязан открыть **актуальный** раздел milestone в `docs/DEFINITION_OF_DONE.md` и проверить **каждый bullet отдельно** против фактического репозитория.

Для каждого пункта сформируй внутреннюю строку:

`<DoD criterion> | STATUS | EVIDENCE`

Допустимые STATUS:

- `PASS_CODE` — подтверждено конкретным файлом/классом/методом;
- `PASS_TEST` — подтверждено конкретным тестом;
- `PASS_CI_BUILD` — подтверждено конкретным CI/build run;
- `PASS_RUNTIME` — подтверждено конкретным emulator/device наблюдением;
- `PASS_OWNER` — критерий по своей природе является ручной/субъективной приёмкой и владелец его подтвердил;
- `DEFERRED_BY_OWNER` — есть явное решение владельца и его ID в `docs/DECISIONS.md`, а критерий внесён в следующий milestone/backlog;
- `FAIL`;
- `UNKNOWN`.

### Неподменяемость доказательств

- `OWNER_ACCEPTANCE` не является доказательством наличия функции в коде.
- Если DoD говорит «есть разбивка по буквам», открой экран/ViewModel/model и убедись, что она реально существует.
- Если DoD говорит «ошибочная буква возвращается позднее», проверь LearningEngine/retry queue и тест на это поведение.
- Если task packet предлагает ADR про Manifest/permission/queries, проверь фактический Manifest до заявления, что решение реализовано.
- CI не заменяет runtime, runtime не заменяет code review, emulator не заменяет physical device, если DoD требует телефон.

### Closure rule

Если хотя бы одна строка = `FAIL` или `UNKNOWN`, результат preflight автоматически:

`PREFLIGHT_STOP_REPO_CONFLICT`

и task packet на закрытие milestone **запрещён**.

Если владелец хочет закрыть milestone без такого критерия, сначала требуется:

1. `ROUTING: OWNER_DECISION`;
2. явное решение владельца о переносе/изменении gate;
3. запись этого решения в `docs/DECISIONS.md`;
4. пометка критерия `DEFERRED_BY_OWNER`, а не `PASS`;
5. перенос критерия в `DEFINITION_OF_DONE.md` следующего milestone и `BACKLOG.md`;
6. только после этого milestone может быть закрыт.

### Обязательный блок в closure task packet

Любой task packet, который закрывает milestone или переносит ветку в `main` как milestone closure, должен содержать:

```text
DOD_AUDIT:
- PASS: <кратко подтверждённые критерии>
- DEFERRED_BY_OWNER: <Dxxx + критерий>, если есть
- FAIL: NONE
- UNKNOWN: NONE
```

Если такого блока нет, **не считай собственный preflight завершённым** и не выдавай task packet до самостоятельного аудита.

## 17. Главный принцип

**Владелец не должен быть техническим контролёром между AI. Ты обязан сам проверять task packet по репозиторию, построчно доказывать DoD перед закрытием milestone, не выдавать самовольные продуктовые решения и использовать Android Studio Agent только когда локальный Android-контекст незаменим.**
