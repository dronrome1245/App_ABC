...
### UX / Audio improvements — D027

- [ ] **[UX-01] Инверсия акцента кнопок на экране итогов сессии по правилу D027.**
  - при `passed == true`: primary `Filled` — `К выбору уровней / Далее`, secondary `Outlined` — `Повторить`;
  - при `passed == false`: primary `Filled` — `Попробовать снова`, secondary `Outlined` — `К выбору уровней`;
...
- [ ] **[AUDIO-02] Подготовка и интеграция обновлённого пакета аудио-ассетов букв v2.**
  - записать/подготовить более мягкий, певучий и протяжный детский/педагогический голос для букв;
  - ориентир длительности одного ассета — `400–700 мс`, с допустимыми обоснованными отклонениями по артикуляции;
  - заменить или версионировать полный комплект letter assets без нарушения существующего `AudioPlayer` / local-first + TTS fallback контракта;
  - проверить mapping всех букв, воспроизведение на устройстве и отсутствие обрезания начала/конца звука.
