...
**Этап:** Stage 0 завершён — продуктовая и управленческая база сформирована. Код приложения ещё не создан.
...
Почему native Android: приложение сначала нужно быстро проверить на Android, а позднее потребуется тесная работа с микрофоном и распознаванием речи. Это уменьшает количество промежуточных библиотек и упрощает работу через Gemini в Android Studio.
...
Сначала сделать **маленькую, полностью рабочую цепочку**, а не сразу весь алфавит и все функции. Первый результат должен запускаться на реальном Android-телефоне и содержать только две буквы, один тип упражнения и сохранение результата. После этого функциональность наращивается небольшими проверяемыми шагами.

`main` после появления кода должен оставаться рабочей веткой. Существенные изменения учебного алгоритма требуют версии LearningPolicy и записи в `docs/DECISIONS.md`.
